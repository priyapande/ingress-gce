...
func init() {
	metrics.DefaultGlobalTracker.SetAggregationStrategy("neg_controller_dual_stack_migration_longest_unfinished_duration_seconds", metrics.StrategyMax)
}
...
// NewNegMetricsCollector returns a SyncerMetrics initialized with the default global registry.
func NewNegMetricsCollector(exportInterval time.Duration, logger klog.Logger) *SyncerMetrics {
	sm, err := NewNegMetricsCollectorWithFactory(exportInterval, metrics.NewStdMetricFactory(prometheus.DefaultRegisterer), logger)
	if err != nil {
		klog.Errorf("Failed to initialize SyncerMetrics: %v", err)
	}
	return sm
}

// NewNegMetricsCollectorWithFactory initializes SyncerMetrics using the given MetricFactory.
func NewNegMetricsCollectorWithFactory(exportInterval time.Duration, factory metrics.MetricFactory, logger klog.Logger) (*SyncerMetrics, error) {
...
func FakeSyncerMetrics() *SyncerMetrics {
	sm, err := NewNegMetricsCollectorWithFactory(5*time.Second, metrics.NewStdMetricFactory(prometheus.NewRegistry()), klog.TODO())
	if err != nil {
